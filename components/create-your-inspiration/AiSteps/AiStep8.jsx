...
export function AiStep8({ setUserChoice, userChoice }) {
    ...
    return (
        <motion.div initial={{ top: 100, opacity: 0 }}
            ...
            <h3 className="font-bold text-2xl md:text-4xl"> Let's give it <span className="text-oBlue-200">Feeling</span></h3>
            ...
            <ul className="flex items-end gap-3 md:gap-6 pt-4 pb-6 md:py-6">
                {
                    feelings.map(({ label, value }) => (
                        <li key={value}>
                            <button onClick={() => selectPosterFeel(value)} className={`px-4 md:px-6 py-3 rounded-md bg-[#F3F3F3] border transition-all ${userChoice.feeling === value ? "border-oBlue-200 font-bold" : "cursor-pointer border-transparent"}`}>
                                {label}
                            </button>
                        </li>
                    ))
