...
export function AiStep2({ setUserChoice, userChoice }) {
    ...
    return (
        <motion.div initial={{ top: 100, opacity: 0 }}
            ...
            <h3 className="font-bold text-2xl md:text-4xl">Let's <span className="text-oBlue-200">Empathize</span></h3>
            ...
            <ul className="flex items-end gap-3 md:gap-6 pt-4 pb-6 md:py-6 flex-wrap">
                {
                    hurdles.map(({ label, value }) => (
                        <li key={value}>
                            <button onClick={() => handleHurdles(label)} className={`px-4 md:px-6 py-3 rounded-md bg-[#F3F3F3] border transition-all ${userChoice.hurdles === label ? "border-oBlue-200 font-bold" : "cursor-pointer border-transparent"}`}>
                                {label}
                            </button>
                        </li>
                    ))
