...
export function AiStep1({ setUserChoice, userChoice }) {
    ...
    return (
        <motion.div initial={{ top: 100, opacity: 0 }}
            ...
            <h3 className="font-bold text-2xl md:text-4xl">Let's Meet <span className="text-oBlue-200">Each Other</span></h3>
            ...
            <ul className="flex items-end gap-3 md:gap-6 pt-4 pb-6 md:py-6 flex-wrap">
                {
                    interests.map(({ label, value }) => {
                        ...
                        return (
                            <li key={value}>
                                <button onClick={() => selectInterests(label)} className={`px-4 md:px-6 py-3 rounded-md bg-[#F3F3F3] border transition-all ${isSelected ? "border-oBlue-200 font-bold" : "cursor-pointer border-transparent"}`}>
                                    {label}
                                </button>
                            </li>
                        )
                ...
                <li>
                    <button onClick={handleClickCustomInterest} className={`px-4 md:px-6 py-3 rounded-md bg-[#F3F3F3] border transition-all ${isAddingInterest ? "border-oBlue-200 font-bold" : "cursor-pointer border-transparent"}`}>
                        Other...
                    </button>
                </li>
            </ul>
