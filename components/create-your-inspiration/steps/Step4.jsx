                ...
                <ul className="flex items-center gap-3 md:gap-10 overflow-scroll pb-4">
                    {
                        fontOptions.map(font => (
                            <li key={font}>
                                <button onClick={() => selectFont(font)} className={`grid border place-items-center rounded-md p-6 w-[120px] aspect-[4/3] ${userChoice.font === font ? "font-bold border-transparent" : " cursor-pointer border-blue-100/20 hover:shadow-xl transition-all"} relative`}>
                                    {font}

                                    {
                                        userChoice.font === font &&
